...
function getHistoryRows() {
  return state.history
    .map(row => {
      const dateValue = row["تاریخ"] || row["بازه زمانی"] || ""
...
function printProgressReport() {
  ...
  const deadlines = getRelevantDeadlineEvents().slice(0, 5)
  const history = getHistoryRows().slice(-6)
  const currentWeek = state.weekly.find(row =>
    ["در حال انجام", "بخشی انجام شد"].includes(row["وضعیت"])
  ) || state.weekly.at(-1) || {}
  ...
  report.innerHTML = `
    <div class="print-report-header">
      <div>
        <span>گزارش پیشرفت رساله دکتری</span>
        <h1>${escapeHTML(config.thesisTitle || "عنوان رساله")}</h1>
    ...
    <div class="print-metrics">
      ${printMetric("پیشرفت فصل دوم", `${faNumber(metrics.chapterTwoPercent)}٪`, `${faNumber(metrics.chapterTwoWords)} کلمه`)}
      ${printMetric("پیشرفت فصل سوم", `${faNumber(metrics.chapterThreePercent)}٪`, `${faNumber(metrics.chapterThreeWords)} کلمه`)}
      ${printMetric("پیشرفت کل رساله", `${faNumber(metrics.thesisPercent)}٪`, "میانگین مراحل اصلی")}
    </div>

    <section class="print-section">
      <h2>مرحله جاری برنامه سالانه</h2>
      <p><strong>${escapeHTML(annual?.["بازه زمانی"] || "ثبت نشده")}</strong></p>
      <p>${escapeHTML(annual?.["تمرکز اصلی"] || "مرحله جاری ثبت نشده است")}</p>
      <p>${escapeHTML(annual?.["خروجی لازم در پایان بازه"] || "")}</p>
    </section>

    <section class="print-section">
      <h2>وضعیت هفته جاری</h2>
      <p><strong>هفته ${faNumber(currentWeek["هفته"] || "—")}</strong>، ${escapeHTML(currentWeek["بازه زمانی"] || "")}</p>
      <p>${escapeHTML(currentWeek["فعالیت اصلی"] || "اطلاعات هفته ثبت نشده است")}</p>
      <p>وضعیت: ${escapeHTML(currentWeek["وضعیت"] || "ثبت نشده")}</p>
    </section>

    <section class="print-section">
      <h2>موعدهای نزدیک و عقب‌افتاده</h2>
      ${deadlines.length
        ? `<ul>${deadlines.map(event => `
            <li>
              <strong>${escapeHTML(event["عنوان"])}</strong>
              <span>${formatPersianDate(event.dateObject)}</span>
            </li>
          `).join("")}</ul>`
        : "<p>موعد نیازمند توجه ثبت نشده است.</p>"}
    </section>

    <section class="print-section">
      <h2>روند اخیر پیشرفت</h2>
      ${history.length
        ? `<table>
            <thead>
              <tr>
            ...
            <tbody>
              ${history.map(row => `
                <tr>
                  <td>${formatPersianDate(row.dateObject)}</td>
                  <td>${faNumber(row.chapterTwo)}٪</td>
                  <td>${faNumber(row.chapterThree)}٪</td>
                  <td>${faNumber(row.thesis)}٪</td>
                </tr>
              `).join("")}
            </tbody>
          </table>`
        : "<p>تاریخچه پیشرفت ثبت نشده است.</p>"}
    </section>

    <section class="print-section">
      <h2>خلاصه منابع و نسخه‌ها</h2>
      <p>منابع ثبت‌شده: ${faNumber(state.sources.length)}</p>
      <p>نسخه‌های ثبت‌شده فصل‌ها و خروجی‌ها: ${faNumber(state.versions.length)}</p>
    </section>
